...
def compute_site_level_probs(df_indiv: pd.DataFrame, output_file: str, num_iterations: int, n_samples: int,
    ...
    grouped = df_indiv.groupby(["chrom", "genomepos", "sample_name"], sort=False)["probability_modified"].apply(np.array)
    ...
    result = pd.DataFrame({
        "chrom": [g[0] for g in grouped.index],
        "genomepos": [g[1] for g in grouped.index],
        "sample_name": [g[2] for g in grouped.index],
...
def main():
    ...
    df_indiv = pd.read_csv(args.indiv_file)
