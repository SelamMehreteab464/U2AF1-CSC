...
def filter_event(args, conditions, inclusion, exclusion):
    ...
    if sum([x >= args.min_event_reads_per_sample for x in totexp]) >= 2:
        if sum([x >= args.min_inc_reads_per_sample for x in inclusion]) >= 1 \
                or sum([x >= args.min_inc_reads_per_sample for x in exclusion]) >= 1:
